let DesktopFunction = function(){
    if($(window).width() >= 1024){
        let ftotHeight = 0;
        let ftotalSlides =  document.querySelectorAll('.feature-overview-list').length;
        let fslideIndex = 1;
        let slideCount = 1;
        ...
        $('.feature-fade-down, .feature-arrows-down').on('click', function(e){
            ...
            if(fslideIndex < ftotalSlides){
                let fcurrentEle = $('.feature-overview-lists').find("[data-count='" + fslideIndex + "']");
            ...
            if(slideCount < ftotalSlides){
                ...
                let targetSlide = $('.feature-overview-image[data-slide='+ slideCount +']').addClass('current-slide');
                if(slideCount === ftotalSlides){
                    slideCount = 1;
                }
            }
        ...
        $('.feature-fade-up, .feature-arrows-up').on('click', function(e){
            ...
            if(fslideIndex <= ftotalSlides){
                ...
                console.log('murali', ftotalSlides);

                $('.feature-overview-image').removeClass('current-slide');
                let targetSlide = $('.feature-overview-image[data-slide='+ fslideIndex +']').addClass('current-slide');
