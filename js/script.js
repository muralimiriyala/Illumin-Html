...
$(window).on('scroll load', function(){
    ...
    if(scroll > 4){ $('.main-header').addClass('fixed_header'); 
    $('.main-header').css({
        'margin-top': 0 + 'px',
    });
}
    else{ $('.main-header').removeClass('fixed_header'); 
    $('.main-header').css({
...
$(document).ready(function(){
    ...
    $('.journey-list').on('click', function(e){
        ...
        $('.product-overview-slide[data-target="' + attrName + '"]').addClass('current-slide');
    });
    ...
    $('.show_more_link').on('click', function(e){
        ...
        $('.corporate-list:nth-child(n+5)').fadeToggle(500).css({'display': 'flex'});
    });
    ...
    $('.leadership-role span').on('click', function(e){
        e.preventDefault();
        $(this).parents().closest('.leadership-list-illumin').siblings().removeClass('expand');
        $(this).parents().closest('.leadership-list-illumin').toggleClass('expand');
        $(this).parents().closest('.leadership-list-illumin').siblings().find('.leadership-desc').fadeOut();
...
if($(window).width() >= 1024){
    $("li.menu-item-has-children.product-menu-item > ul.sub-menu").each(function() {
